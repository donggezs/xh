...
$("body").on("click", '#yx-wendajia',function () {
    ...
    console.log(parse_cookie)
    let sign = h(parse_cookie.split("_")[0] + '&' + t + '&' + 12574478 + '&' + data)

    $.ajax({
        url: "https://h5api.m.tmall.com/h5/mtop.taobao.social.feed.aggregate/1.0/?jsv=2.4.8&appKey=12574478&sign="+sign+"&t="+t+"&api=mtop.taobao.social.feed.aggregate&v=1.0&ecode=1&type=jsonp&dataType=jsonp&data="+encodeURIComponent(data),
        dataType: "jsonp",
        type: "jsonp",
    ...
    window.mtopjsonp1 = function (v) {
        ...
        allList.map(v => { 
            ...
            window[name] = function (val) {
                ...
                if (ListNum == allList.length) {
                    ...
                    xin.map(p => { 
                        ...
                        $.ajax({
                            url: "https://h5api.m.tmall.com/h5/mtop.taobao.social.ugc.post.detail/2.0/?jsv=2.4.8&appKey=12574478&sign="+q_sign+"&t="+q_t+"&api=mtop.taobao.social.ugc.post.detail&v=2.0&ecode=1&type=jsonp&dataType=jsonp&data="+encodeURIComponent(q_data),
                            dataType: "jsonp",
                            type: "jsonp",
            ...
            $.ajax({
                url: "https://h5api.m.tmall.com/h5/mtop.taobao.social.ugc.post.detail/2.0/?jsv=2.4.8&appKey=12574478&sign="+sign+"&t="+t+"&api=mtop.taobao.social.ugc.post.detail&v=2.0&ecode=1&type=jsonp&dataType=jsonp&data="+encodeURIComponent(data),
                dataType: "jsonp",
                type: "jsonp",
                jsonpCallback: name
            })
...
$("body").on("click", "#yx-download-img-video", function () {
    ...
    setTimeout(function(){
        ...
        $(".tb-img>li").each(function () { 
            let str = $(this).find("a").eq(0).attr("style")
            if (str) { 
                let str1 = str.split("(")[1]
                let str2 = str1.split(")")[0]

                str2 = str2.replace("40x40", "800x800")
                imgAll.push(str2)
                zhutuLen++
            }
        })
    ...
    for (let i = 1; i < 100; i++) {
        ...
        window[name] = function (v) {
            ...
            if (qqNum == 99) {
                ...
                let allSku,skuList,span
                if(totalList.length > 0){
                    allSku = totalList[0].auctionSku.split(";")
                    skuList = []
                    span = ''
                    allSku.map((v, i) => { 
...
$("body").on("click", ".yx-sp1", function(){
    layui.use('table', function () {
        var table = layui.table;
        table.exportFile(pieTableRender.config.id, pies1, 'xls'); //导出数据
    })
})
...
$("body").on("click", ".yx-sp1_2", function () {
    layui.use('table', function () {
        var table = layui.table;
        table.exportFile(pieTableRender1.config.id, pies2, 'xls'); //导出数据
    })
})
...
$("body").on("click", ".yx-sp1_3", function () {
    layui.use('table', function () {
        var table = layui.table;
        table.exportFile(pieTableRender2.config.id, pies3, 'xls'); //导出数据
    })
})
...
$("body").on("click", ".yx-sp1_4", function () {
    layui.use('table', function () {
        var table = layui.table;
        table.exportFile(pieTableRenderXiu.config.id, series3, 'xls'); //导出数据
    })
})
...
$("body").on("click", ".yx-sp1_5", function () {
    layui.use('table', function () {
        var table = layui.table;
        table.exportFile(pieTableRenderWen.config.id, series_wen, 'xls'); //导出数据
    })
})
